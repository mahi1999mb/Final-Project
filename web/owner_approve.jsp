...
<%
    ...
    try {
        int i = sto.executeUpdate("update owner_reg set status='Approved'  where id='" + id + "' ");
        System.out.println("test print==" + id);
        if (i != 0) {
            ResultSet rs = st.executeQuery(" SELECT * from owner_reg where id = '" + id + "' ");
            if (rs.next()) {
                ...
                System.out.println("success");
                response.sendRedirect("View_owners1.jsp?Granted");
            } else {

                System.out.println("failed");
                response.sendRedirect("View_owners1.jsp?Failed");
            }
        }
    } catch (Exception ex) {
        ex.printStackTrace();
